...
class Layout:
    def __init__(self, rect: list[float], layout_params: LayoutParams | None = None):
        self.rect = rect
...
class LinearLayout(Layout):
    def __init__(self, direction: int, rect: list[float], layout_params: LayoutParams | None = None):
        super().__init__(rect, layout_params)
        self.direction = direction
        self.center = Vector(self.rect[:2]) + 0.5 * Vector(self.rect[2:])

    def apply(self, elements: list[GUIElement]):
        print(elements, elements[-1].rect)
        content_size = 0
...
class VerticalLayout(LinearLayout):
    def __init__(self, rect: list[float], layout_params: LayoutParams | None = None):
        super().__init__(1, rect, layout_params)

class HorizontalLayout(LinearLayout):
    def __init__(self, rect: list[float], layout_params: LayoutParams | None = None):
        super().__init__(0, rect, layout_params)